...
def get_full_analysis(expected_str, predicted_ids, frames, audio_len, processor):
    """Combines your Cell 3 and Cell 4 logic"""
    # 1. Generate Raw Timestamps
    raw_user_phonemes = phoneme_timestamps(predicted_ids, frames, audio_len, processor)
    
    # 2. Filter segments > 0.01s and Map to Friendly Labels (Cell 3 logic)
    processed_segments = []
    detected_for_acc = []
    for token, start, end in raw_user_phonemes:
        duration = end - start
        if duration > 0.01:
          
            detected_for_acc.append(token)

    # 3. Accuracy Calculation (Cell 4 logic)
    ...
    # 4. Substitution Feedback (Cell 4 logic)
    feedback = []
    det_set = set(detected_for_acc)
    for ref_ph in expected:
        if any(is_equivalent(ref_ph, d) for d in detected_for_acc): continue
        ...
        if sub:
            ...
            if specific_message:
                feedback.append({
                    ...
                    "messege": (
                        f'Used "{PHONEME_TO_ARABIC.get(sub, sub)}" '
                        f'instead of "{PHONEME_TO_ARABIC.get(ref_ph, ref_ph)}"'
                    )
                })
    return accuracy, feedback, processed_segments, expected, detected_for_acc